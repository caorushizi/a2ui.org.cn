...
本路线图概述了 A2UI 项目的当前状态和未来计划。该项目处于积极开发中，优先事项可能会根据社区反馈和新出现的用例而调整。
...
| **React** | 🚧 进行中 | Web | 即将发布于 2026 年第一季度 |
...
| 代理 UI 工具包 | 状态 | 说明 |
|-------------|--------|-------|
| **CopilotKit** | ✅ 完成 | 感谢 AG UI 带来的零日兼容性 |
...
| 集成 | 状态 | 说明 |
|-------------|--------|-------|
| **任何支持 A2A 的代理** | ✅ 完成 | 感谢 A2A 协议带来的零日兼容性 |
| **ADK** | 📋 计划中 | 仍在设计开发者工效，参见 [samples](https://github.com/google/A2UI/tree/main/samples/agent/adk) |
| **Genkit** | 💡 提议中 | 社区兴趣 |
| **LangGraph** | 💡 提议中 | 社区兴趣 |
| **CrewAI** | 💡 提议中 | 社区兴趣 |
| **AG2** | 💡 提议中 | 社区兴趣 |
...
- v0.8.0 规范发布
- A2A 扩展（感谢 Google A2A 团队！在 [a2asummit.ai](https://a2asummit.ai) 预告）
...
一个具有基于 Hook API 和完整 TypeScript 支持的原生 React 渲染器。
...
- **更多代理框架**：将你的代理框架映射到 A2UI
- **更多传输层**：将你的传输层映射到 A2UI
- **社区组件库**：与社区共享自定义组件
- **社区示例**：与社区共享自定义示例
- **社区评估**：生成式 UI 评估场景和标记数据集
- **开发者工效**：如果你能构建更好的 A2UI 体验，请与社区分享

## 如何影响路线图

我们欢迎社区对优先事项提出意见：

1. **为 Issue 投票**：给你关心的 GitHub issue 点赞 👍
2. **提议功能**：在 GitHub 上开启讨论（请先搜索现有讨论）
3. **提交 PR**：构建你需要的功能（请先搜索现有 PR）
4. **加入讨论**：分享你的用例和需求（请先搜索现有讨论）

## 发布周期

- **主版本** (1.0, 2.0)：每年发布，或在需要进行重大不兼容更改时发布
- **次版本** (1.1, 1.2)：每季度发布，包含新功能
- **补丁版本** (1.1.1, 1.1.2)：根据需要发布，用于错误修复
