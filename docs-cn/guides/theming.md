...
- **代理描述 *显示什么*** (组件和结构)
- **客户端决定 *看起来如何*** (颜色、字体、间距)

这确保了：

- ✅ **品牌一致性**: 所有 UI 都匹配你的应用设计系统
- ✅ **安全性**: 代理无法注入任意 CSS 或样式
- ✅ **无障碍性**: 你控制对比度、焦点状态和 ARIA 属性
...
flowchart TD
    ...
    B["2. 主题配置<br/>你配置<br/>(颜色, 字体, 间距)"]
    C["3. 组件覆盖<br/>你定制<br/>(特定组件的 CSS/样式)"]
    D["4. 渲染输出<br/>原生平台小部件"]
...
- 其他组件有它们自己的提示（参见 [组件参考](../reference/components.md)）

客户端渲染器根据你的主题和设计系统将这些语义提示映射到实际的视觉样式。

## 第 2 层：主题配置

每个渲染器都提供了一种全局配置你的设计系统的方法，包括：

- **颜色**: 主色、次色、背景色、表面色、错误色、成功色等。
- **排版**: 字体系列、大小、字重、行高
- **间距**: 基础单位和比例 (xs, sm, md, lg, xl)
- **形状**: 边框半径值
- **高度 (Elevation)**: 用于深度的阴影样式
...
- **[自定义组件](custom-components.md)**: 使用你的样式构建自定义组件
