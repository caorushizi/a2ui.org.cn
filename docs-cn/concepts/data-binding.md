...
数据绑定使用 JSON Pointer 路径 ([RFC 6901](https://tools.ietf.org/html/rfc6901)) 将 UI 组件连接到应用程序状态。这使得 A2UI 能够高效地为大型数组定义布局，或显示更新的内容而无需从头重新生成。

## 结构与状态

A2UI 分离了：

1. **UI 结构** (组件): 界面看起来是什么样子
...
**更新:** 发送带有 `status: "Shipped"` 的 `dataModelUpdate` → 显示 "Shipped"
